...
The design intent is to keep the component small, visually contained, and easy to scan. Badge supports a limited tone set for neutral/default values, brand emphasis, positive values, and negative values. Content should be short, usually one to four characters, such as `1`, `+1`, or `-1`.
